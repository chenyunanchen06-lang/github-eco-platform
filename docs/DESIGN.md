...
**全量实测（2026-09-06 ~ 09-13，共 8 天 192 个小时文件）**

| 指标 | 实测值 |
|---|---|
| 原始数据 | **192 个文件 / 7,415 MB（压缩）** |
| **事件总量** | **14,910,004 条**（ODS 解析失败 0 条） |
| ODS Parquet | 6,791.7 MB（gz → Parquet 比 1.1x） |
| 单条平均体积 | 478 字节 |
| event_id 唯一率 | **99.9999%**（14,906,594 中重复 17 条） |
| 关键字段空值 | **0**（event_id / event_type / created_at） |
| 时间戳完整性 | 8 天 × 24 小时分区全满 |
| 维度基数 | 2,431,749 个仓库 / 1,594,347 个用户 |
| 单日事件量 | **107 万 ~ 283 万条（波动 2.64x）** |
| 事实表（DWD） | 14,906,594 行 / 519 MB / 8 个 dt 分区 |
| ADS 六张宽表 | 6,227,560 行 |

> ⚠️ **日间波动远超直觉**：环比波动中位数 **24.9%**，最大 **86.3%**，
> 全局最高/最低比 **2.64x**。这不是数据质量问题 —— GitHub Archive 本身
> 就是这样。**任何数据质量阈值都必须按实测分布标定**，见 §5 的 R4。
>
> ⚠️ **体积波动更大**：另测 `2026-09-01-0` 仅 6.9 MB（5.2 万条），
> 与常规小时差近 9 倍。不要照搬任何人的规模数字，按自己的日期范围实测。
>
> ⚠️ **磁盘预留**：8 天原始 7.4 GB + ODS 6.8 GB + DWD 约 0.6 GB ≈ **15 GB**。
...
| R4 波动 | dwd.gh_events_detail | 相邻日行数波动 | < 100% | 告警 |
...
实测结果：**5/5 通过**（14,910,004 行，重复 17 条，0 空值，8 天分区全满）。

> **⚠️ R4 的阈值是量出来的，不是拍脑袋定的 —— 这条最值得讲**
>
> 最初按业界习惯写了 **±30%**。实测 8 天的环比波动是：
> 最小 3.5%、**中位数 24.9%**、最大 86.3%，均值 35.4%、标准差 30.9%。
> 也就是说**中位数就已经贴着 30%**——这条规则会隔一天告警一次，
> 属于典型的「告警疲劳」，等于没有监控。
>
> 按 **mean + 2σ ≈ 97.3%** 重新标定为 **100%**（日环比翻倍才算异常）。
>
> **面试价值**：能主动说出"我把阈值从 30% 改成 100%，因为实测中位数就是 24.9%"，
> 证明你理解 DQ 规则的目的是**发现异常**而不是**制造噪音**。
> 绝大多数候选人只会背"我们做了数据质量监控"。
